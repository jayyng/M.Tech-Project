...
def nbkmh(train_index, test_index):
    ...
    for index,row in x_kmeans.iterrows():
        dataPts.append((row[0],row[1],row[2]))
        u=(0,0,0)
        result.append(distance.euclidean(dataPts[index],u))
    for i in range(len(result)):
        dictOfDataPts[result[i]]=dataPts[i]
    for key in sorted(dict.keys(dictOfDataPts)):
        sortedDataPts.append(dictOfDataPts[key])
    partitions=(chunkIt(sortedDataPts,clusters))
    for i in range(clusters):
        x=0
        y=0
        z=0
        for j in range(len(partitions)):
            x=x+partitions[i][j][0]
            y=y+partitions[i][j][1]
            z=z+partitions[i][j][2]
        meanOfx=x/len(partitions)
        meanOfy=y/len(partitions)
        meanOfz=z/len(partitions)
        ct=(meanOfx,meanOfy,meanOfz)
        Centroid.append(ct)
